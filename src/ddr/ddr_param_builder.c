 ...
 * Generated by analyzing /references/cloner-2.5.43-ubuntu_thingino/bin/64bit/cloner_64
 * 
...
static const ddr_chip_config_t t31x_config = {
    .chip_type = CHIP_T31X,
...
static const ddr_chip_config_t t31nl_config = {
    .chip_type = CHIP_T31L,
    .ddr_type = DDR_TYPE_DDR2,
...
// Likely identical to T31X for DDR generation
static const ddr_chip_config_t t23n_config = {
    .chip_type = CHIP_T23N,
    .ddr_type = DDR_TYPE_DDR2,
...
int ddr_get_chip_config(chip_type_t chip, ddr_chip_config_t *config) {
    ...
    switch (chip) {
        case CHIP_T31X:
            memcpy(config, &t31x_config, sizeof(ddr_chip_config_t));
            return 0;
        case CHIP_T31L:
            memcpy(config, &t31nl_config, sizeof(ddr_chip_config_t));
            return 0;
        case CHIP_T23N:
            memcpy(config, &t23n_config, sizeof(ddr_chip_config_t));
            return 0;
        default:
            fprintf(stderr, "ERROR: Unsupported chip type: 0x%x\n", chip);
            return -1;
    }
}

int ddr_build_params(const ddr_chip_config_t *config, ddr_params_t *params) {
    if (!config || !params) return -1;
