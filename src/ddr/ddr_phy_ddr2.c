 ...
 * Based on decompiled DDR2Param::ddrp_generate_register @ 0x0044e090
 * 
...
int ddr_generate_ddrp_ddr2(const ddr_config_t *config, uint8_t *obj_buffer, uint8_t *ddrp_regs) {
    ...
    *(uint32_t *)(obj_buffer + 0x11c) = clock_mhz * 1000000;
    ...
    // Offsets verified from TXX_DDRBaseParam::ddrc_generate_register @ 0x00471890
    ...
    *(uint32_t *)(obj_buffer + 0x1dc) = 0x00002000;  // Default value
    ...
    // ========================================
    // From TXX_DDR2Param::ddr_fill_chip_param @ 0x00473ad0
    ...
    if (data_width == 4) {
        ...
        printf("[DDR2] Invalid data width: %u (expected 4, 8, or 16)\n", data_width);
        return -1;
    }
    ...
    // ========================================
    // From TXX_DDR2Param::ddr_fill_chip_param @ 0x00473a7f
    // TXX chips write CAS to obj[0x1d4] bits [6:4]

    if (cas_latency < 2 || cas_latency > 7) {
        printf("[DDR2] CAS latency out of range: %u (valid: 2-7)\n", cas_latency);
        return -1;
    }
    ...
    // ========================================
    // From TXX_DDR2Param::ddr_fill_chip_param @ 0x004739c5
    ...
    if (tWR_cycles > 8) {
        ...
        printf("[DDR2] tWR too small: %u cycles (minimum: 2)\n", tWR_cycles);
        return -1;
    }
    ...
    // ========================================
    // From decompilation at 0x0044e108
    ...
    obj_buffer[DDR_PHY_REG_ODT1] = (obj_buffer[DDR_PHY_REG_ODT1] & 0xb8) |
                                    (odt1 & 0x01) |
                                    ((odt2 & 0x01) << 1) |
                                    ((odt3 & 0x01) << 2) |
                                    (((odt3 & 0x02) != 0) << 6);
    ...
    // ========================================
    // From decompilation at 0x0044e120
    ...
    *(uint32_t *)(obj_buffer + DDR_PHY_REG_EXT_TIMING1) = 
        (*(uint32_t *)(obj_buffer + DDR_PHY_REG_EXT_TIMING1) & 0xfff80000) | 
        (ext_timing_1 & 0x7ffff);
    ...
    *(uint16_t *)(obj_buffer + DDR_PHY_REG_EXT_TIMING2) = 
        (*(uint16_t *)(obj_buffer + DDR_PHY_REG_EXT_TIMING2) & 0xf807) | 
        ((ext_timing_2 & 0x1ff) << 3);
    
    // ========================================
    // STEP 6: tRL (Read Latency) Related
    // ========================================
    // From decompilation at 0x0044e140
    
    uint32_t tRL_cycles = ps2cycle_ceil(config->tRL, clock_mhz);
    if (tRL_cycles < 2 || tRL_cycles > 3) {
        printf("[DDR2] tRL out of range: %u cycles (valid: 2-3)\n", tRL_cycles);
        return -1;
    }
    
    obj_buffer[DDR_PHY_REG_BASE_START] = (obj_buffer[DDR_PHY_REG_BASE_START] & 0xfc) | 
                                          (tRL_cycles & 0x03);
    
    // ========================================
    // STEP 7: Register Impedance
    // ========================================
    // From decompilation at 0x0044e1e5
    ...
    *(uint32_t *)(obj_buffer + DDR_PHY_REG_IMPEDANCE) = 
        0x01802e02 | ((impedance_high * 2 | impedance_low) << 18);
    
    // ========================================
    // STEP 8: Copy to Output Format using TXX-specific ddr_convert_param mapping
    // ========================================
    // From Ghidra decompilation of TXX_DDRBaseParam::ddr_convert_param @ 0x0046ba40:
    ...
    // TODO: Implement proper DDRP mapping from Ghidra decompilation
    // For now, use hardcoded reference values from the working binary
    // Reference values for 128MB DDR2 @ 400MHz (from Ghidra tool output):
