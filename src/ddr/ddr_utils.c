...
uint8_t ddr_get_phy_type_field(ddr_type_t type) {
    switch (type) {
        case DDR_TYPE_DDR2:    return 3;
        case DDR_TYPE_DDR3:    return 0;
        case DDR_TYPE_LPDDR:   return 4;
        case DDR_TYPE_LPDDR2:  return 4;
        case DDR_TYPE_LPDDR3:  return 2;
        default:               return 0;
    }
}
...
uint8_t ddr_get_ctrl_type_field(ddr_type_t type) {
    switch (type) {
        case DDR_TYPE_DDR2:    return 0x2;   // 10 in binary (bits 2-3)
        case DDR_TYPE_DDR3:    return 0x1;   // 01 in binary
        case DDR_TYPE_LPDDR:   return 0x3;   // 11 in binary
        case DDR_TYPE_LPDDR2:  return 0x3;   // 11 in binary
        case DDR_TYPE_LPDDR3:  return 0x3;   // 11 in binary
        default:               return 0;
    }
}
...
int ddr_validate_timing(const char *param_name, uint32_t value, uint32_t min_val, uint32_t max_val) {
    if (value < min_val || value > max_val) {
        printf("[ERROR] [DDR] %s out of bounds: %u (min:%u, max:%u)\n", 
